...
void atrousFilterCudaNoOpt(GBuffer<uchar3> frame, int depth, FilterParams params, cudaStream_t stream){
    for(int i = 0; i < depth; i++){
        dim3 blockShape = dim3(32, 4);
...
KERNEL void atrousFilterCudaKernelNoOpt(const uchar3* in, uchar3* out, int level, GBuffer<uchar3> frame, FilterParams params){
    ...
    for(dPos.x = -ATROUS_RADIUS; dPos.x <= ATROUS_RADIUS; dPos.x++){
        for(dPos.y = -ATROUS_RADIUS; dPos.y <= ATROUS_RADIUS; dPos.y++){
            ...
            float3 nRender = make_float3(frame.render[posToIndex(nFramePos, frame.shape)]);
    ...
    out[posToIndex(pos, frame.shape)] = make_type<uchar3>(acum);
}

CUDA_FUNC void atrousFilterPixelNoOpt(int2 pos, const uchar3* in, uchar3* out, int level, GBuffer<uchar3> frame, FilterParams params){
    const float waveletSpline[3] = {3.0/8.0, 1.0/4.0, 1.0/16.0};
