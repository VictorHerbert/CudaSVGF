...
__inline__ CUDA_FUNC void saveTileAligned(uchar4* tile, int2 tileShape, const uchar4* img, int2 imgShape, int2 tileStartPos){
...
void atrousFilterCudaTileAligned(GFrame<uchar4> frame, int depth, FilterParams params, cudaStream_t stream){
    for(int i = 0; i < depth; i++){
        dim3 blockShape;
        dim3 gridShape((frame.shape.x + blockShape.x-1) / blockShape.x, (frame.shape.y + blockShape.y-1) / blockShape.y);

        atrousFilterCudaKernelTileAligned<<<gridShape, blockShape, 0, stream>>>(
...
KERNEL void atrousFilterCudaKernelTileAligned(const uchar4* in, uchar4* out, int level, GFrame<uchar4> frame, FilterParams params){
    int2 framePos = {blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y};
    extern __shared__ uchar4 sharedMem[];

    atrousFilterPixelTileAligned(framePos, sharedMem, in, out, level, frame, params);
}

CUDA_FUNC void atrousFilterPixelTileAligned(int2 pos, uchar4* tile, const uchar4* in, uchar4* out, int level, GFrame<uchar4> frame, FilterParams params){
    ...
    int (*d)[ATROUS_DIM] = (int (*)[ATROUS_DIM])tile;
    ...
    saveTileAligned(tile, tileShape, frame.albedo, frame.shape, tileStart);
    ...
    for(dPos.x = -ATROUS_RADIUS; dPos.x <= ATROUS_RADIUS; dPos.x++){
        for(dPos.y = -ATROUS_RADIUS; dPos.y <= ATROUS_RADIUS; dPos.y++){
            ...
            int nMemIdx = flattenIndex(nFramePos - tileStart, tileShape);
    ...
    saveTileAligned(tile, tileShape, frame.normal, frame.shape, tileStart);
