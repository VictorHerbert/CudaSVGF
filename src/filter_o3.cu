...
CUDA_CPU_FUNC void atrousFilterPixelO3(int2 pos, const uchar4* in, uchar4* out, int level, GBuffer<uchar4> frame, FilterParams params){
...
KERNEL void atrousFilterCudaKernelO3(const uchar4* in, uchar4* out, int level, GBuffer<uchar4> frame, FilterParams params){
    ...
    atrousFilterPixelO3(framePos, in, out, level, frame, params);
}

void atrousFilterCudaO3(GBuffer<uchar4> frame, int depth, FilterParams params, cudaStream_t stream){
    for(int i = 0; i < depth; i++){
        dim3 blockShape;
        dim3 gridShape((frame.shape.x + blockShape.x-1) / blockShape.x, (frame.shape.y + blockShape.y-1) / blockShape.y);

        atrousFilterCudaKernelO3<<<gridShape, blockShape, 0, stream>>>(
